    ...
    try {
      ...
      for (const resource of matchingResources) {
        ...
        await new Promise((resolve, reject) => {
          ...
          const trackListener = (emitter, event, handler, preferOnce = true) => {
            if (!emitter) return;
            if (preferOnce && typeof emitter.once === 'function' && typeof emitter.removeListener === 'function') {
              emitter.once(event, handler);
              detachListeners.push(() => emitter.removeListener(event, handler));
              return;
            }
            if (typeof emitter.on === 'function') {
              ...
              if (typeof emitter.removeListener === 'function') {
                ...
                detachListeners.push(() => emitter.off(event, handler));
              } else {
                detachListeners.push(() => {});
              }
              return;
            }
            if (preferOnce && typeof emitter.addEventListener === 'function' && typeof emitter.removeEventListener === 'function') {
              emitter.addEventListener(event, handler, { once: true });
              detachListeners.push(() => emitter.removeEventListener(event, handler));
            }
          };

          function cleanup() {
            while (detachListeners.length) {
              const remove = detachListeners.pop();
              try {
                remove();
              } catch (err) {
                debug('Failed to detach listener:', err && err.message ? err.message : err);
              }
            }
          }
          ...
          trackListener(res, 'finish', onResponseFinished);
          trackListener(req, 'close', onResponseClosed);
          trackListener(req, 'aborted', onResponseClosed);
