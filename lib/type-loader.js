...
async function loadCustomResource(basepath, file, types) {
  try {
    // Resolve from the perspective of the app (basepath)
    const { createRequire } = require('module');
    const appRequire = createRequire(path.resolve(basepath, 'package.json'));
    const resolvedPath = appRequire.resolve(file);
    const customResource = appRequire(file);

    // Determine if export looks like a Deployd Resource subclass
    let looksLikeResource = false;
    if (customResource && customResource.prototype) {
      try {
        // Back-compat flag set by Resource constructor on instances in older code
        looksLikeResource =
          !!customResource.prototype.__resource__;

        if (!looksLikeResource) {
          // Fallback: check prototype chain against core Resource prototype
          const CoreResource = require('./resource');
          looksLikeResource = CoreResource && CoreResource.prototype && CoreResource.prototype.isPrototypeOf(customResource.prototype);
        }
      } catch (e) {
        // ignore and treat as not a resource
      }
    }
    ...
    } else if (customResource && typeof customResource.selfHost === 'function') {
      // Allow self-hosting modules (like dpd-dashboard) even if not a Resource subclass
      types[customResource.name || file] = customResource;
      debug('Loaded self-hosting module:', customResource.name || file);
    } else {
