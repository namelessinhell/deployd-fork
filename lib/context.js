...
class Context {
  ...
  constructor(resource, req, res, server) {
    ...
    this.url = this._extractUrl(resource.path, req.url);
    ...
    this.dpd = internalClient.build(server, req.session, req.stack, this);
  }
  ...
  _handleRecursionLimit() {
    const limit = this.query.$limitRecursion || this.body.$limitRecursion;
    if (limit !== undefined) {
