...
function loadResourceDir(basepath, fn) {
  ...
  try {
    var folders = shell.find(resourceDir).filter(function (file) {
...
function addInternalResources(getTypes, basepath, server, resources, fn) {
  var absoluteBasepath = path.resolve(basepath);
  new Promise(function (resolve) {
    ...
    if (!path.isAbsolute(publicFolder)) {
      var resolvedPublic = path.resolve(publicFolder);
      if (resolvedPublic.indexOf(absoluteBasepath) === 0) {
        publicFolder = resolvedPublic;
      } else {
        publicFolder = path.join(absoluteBasepath, publicFolder);
      }
    }
    ...
    if (fs.existsSync(staticFolder) && fs.statSync(staticFolder).isDirectory()) {
      internals.push(new Files('static', { config: { 'public': staticFolder }, server: server }));
    }
