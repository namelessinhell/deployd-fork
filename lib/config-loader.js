...
class ConfigLoader {
  ...
  constructor(server) {
    this.server = server;
    this.resourceCache = server.__resourceCache || [];
  }
  ...
  async loadTypes() {
    ...
    try {
      const { defaults, types } = await _loadTypes();
...
module.exports = new ConfigLoader(require('./server')); // Assuming server is exported from './server'
