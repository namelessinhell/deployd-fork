...
// Support both v5-v6 style (module is constructor) and v7+ (named export LRUCache)
const LRUExport = require('lru-cache');
const LRUCache = LRUExport && (LRUExport.LRUCache || LRUExport);
...
class Script {
  ...
  constructor(src, filePath) {
    ...
    // Replace unlimited memoization with LRU cache for better memory management
    this.functionCache = new LRUCache({
      max: 100,
      // Prefer ttl (v7+) but keep maxAge for older versions
      ttl: 1000 * 60 * 60,
      maxAge: 1000 * 60 * 60,
      updateAgeOnGet: true
    });

    this.getFunction = this._getCachedFunction.bind(this);
  }

  /**
   * Gets a cached function or creates a new one if not in cache.
   *
   * @private
   * @param {Array<String>} functionArgs - The argument names for the function.
   * @returns {Function} - The cached or newly created Function instance.
   */
  _getCachedFunction(functionArgs) {
    const cacheKey = JSON.stringify(functionArgs);
    let cachedFunction = this.functionCache.get(cacheKey);

    if (!cachedFunction) {
      cachedFunction = this._createFunction(functionArgs);
      this.functionCache.set(cacheKey, cachedFunction);
      debug(`Created new function for args: ${cacheKey}`);
    } else {
      debug(`Using cached function for args: ${cacheKey}`);
    }
  ...
  async run(ctx, domain, fn) {
    ...
    // Track async callbacks on the context so event handlers can update them
    scriptContext.waitingForCallback = false;
    scriptContext.callbackCount = 0;
    let isDone = false;
    let events;
    ...
    // Expose done for cancel/flow helpers
    this.done = doneCallback;

    if (domain) {
      const wrapAsync = typeof fn === 'function';
      this._setupDomainHandlers(domain, scriptContext, events, doneCallback, wrapAsync);
    }
    ...
    process.nextTick(() => {
      if (!scriptContext.waitingForCallback && (scriptContext.callbackCount || 0) <= 0) {
   ...
   */
  _setupDomainHandlers(domain, scriptContext, events, doneCallback, wrapAsync) {
  ...
  _wrapPromise(promiseable, sandbox, events, done, sandboxRoot) {
    ...
    // Only wrap if not already a promise to avoid unnecessary overhead
    ...
    if (!ret) {
      // Only cast if not already a promise
      if (!this._isPromise(realPromise)) {
  ...
  static async load(filePath, fn) {
    ...
    // Promise style
    const src = await fs.readFile(filePath, 'utf-8');
    return new Script(src, filePath);
  }
}
